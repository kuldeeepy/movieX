...
async function getMovieContent(ops) {
  ...
  for (let idx = 0; idx < Math.min(iteration, video.results.length); idx++) {
    if (
      video.results[idx].name.toLowerCase().includes("trailer") &&
      video.results[idx].key
    ) {
  ...
  const movieContent = {
    ...
    production:
      info.production_companies.map((prod) => prod.name).join(", ") || "N/A",
    lang:
...
function displayContent(movie) {
  ...
  mainPage.innerHTML += `
        ...
        <div>
            <div class='details'>
            <img src="${
              imageUrl + movie.imageTwo || imageUrl + movie.imageOne
            }" />
            <div>
                <h1>${movie.name}</h1>
                <div>
...
async function recommendation() {
  ...
  let resp = await fetch(
    `https://api.themoviedb.org/3/tv/top_rated?language=en-US&page=${Math.floor(
      Math.random() * 100
    )}`,
    ops
  );
