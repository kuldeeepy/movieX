...
export async function getUpcoming() {
  let resp1 = await fetch(
    `${apiUrl}discover/tv?api_key=${key}&sort_by=release_date.desc&page=1`
  );
  let resp2 = await fetch(
    `${apiUrl}discover/tv?api_key=${key}&sort_by=release_date.desc&page=2`
  );
...
export async function getLatestShows() {
  let resp1 = await fetch(
    `${apiUrl}trending/tv/day?language=en-US&page=${
...
export async function getLatestMovies() {
  let resp1 = await fetch(
    `${apiUrl}movie/now_playing?language=en-US&page=${
      Math.floor(Math.random() * 10) + 1
    }`,
    options
  );
...
export async function getTrending() {
  let resp1 = await fetch(
    `${apiUrl}movie/popular?language=en-US&page=${
...
export async function getDiscover() {
  let response = await fetch(
    `${apiUrl}discover/tv?api_key=${key}&with_networks=${
      networks[Math.floor(Math.random() * 4)]
    }`
  );
